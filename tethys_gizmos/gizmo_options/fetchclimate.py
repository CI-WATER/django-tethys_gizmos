...
class FetchClimateMapData(TethysGizmoOptions):
    ...
    drawing_types_enabled(string array): A list of the types of geometries the user will be allowed to draw.
    Valid types are: RECTANGLE, and POINTS. (e.g.: drawing_types_enabled=['RECTANGLE','POINTS'])
    initial_drawing_mode(string array): A string representing the drawing mode that will be enabled by default.
...
class FetchClimateMapParameters(TethysGizmoOptions):
    ...
    Attributes
    css(dictionary):Custom css elements. FORMAT:{'css-element-name': 'css-value'}.
...
class FetchClimatePlotParameters(TethysGizmoOptions):
    ...
    Attributes
    dimensions(string dictionary): The integer is in pixels for width (`Highcharts width reference
